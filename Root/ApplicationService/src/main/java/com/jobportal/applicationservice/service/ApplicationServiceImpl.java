...
public class ApplicationServiceImpl implements ApplicationService {
    ...
    @Override
    public List<JobApplicationResponse> getJobApplications(Long jobId, String role,Long recruiterId) {
        ...
        // Fetch Job details from JobService to check ownership
        JobResponse job = jobClient.getJobById(jobId);
        
        // Validate Ownership
        if(!job.getRecruiterId().equals(recruiterId)) {
        	throw new UnauthorizedException("Access Denied! You can view applications for your own jobs.");
        }
        ...
        return applicationRepository.findByJobId(jobId)
                ...
                .map(app -> {
                    ...
                    try {
                        UserResponse user =userClient.getUserById(app.getUserId(),internalSecret);
    ...
    @Override
    public ApplicationResponse updateStatus(Long applicationId, ApplicationStatus status,Long recruiterId, String role) {
        ...
        // find the applocation
        JobApplication application = applicationRepository.findById(applicationId)
        		.orElseThrow(() -> new ApplicationNotFoundException("Application not found with id: "+applicationId));

        // Fetch the assocoiated job to check if the recruiter owns it
        JobResponse job = jobClient.getJobById(application.getJobId());
        
        // Validate ownership
        if(!job.getRecruiterId().equals(recruiterId)) {
        	throw new UnauthorizedException("Access Denied! You can only update applications for your own jobs.");
        }
        ...
        ApplicationResponse response = modelMapper.map(updated,
                        ApplicationResponse.class);
